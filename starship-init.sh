# Output of:
# /opt/homebrew/bin/starship init bash --print-full-init
# Using starship 1.21.1 (because I don't want to eval the internet)
...
starship_precmd() {
    ...
    PS1="$(/opt/homebrew/bin/starship prompt "${ARGS[@]}")"
    if [[ ${BLE_ATTACHED-} ]]; then
        local nlns=${PS1//[!$'\n']}
        bleopt prompt_rps1="$nlns$(/opt/homebrew/bin/starship prompt --right "${ARGS[@]}")"
    fi
...
# then hook our functions into their framework.
    ...
    if [[ -n "${BASH_VERSION-}" ]] && [[ "${BASH_VERSINFO[0]}" -gt 4 || ( "${BASH_VERSINFO[0]}" -eq 4 && "${BASH_VERSINFO[1]}" -ge 4 ) ]]; then
        starship_preexec_ps0() {
            /opt/homebrew/bin/starship time
        }
